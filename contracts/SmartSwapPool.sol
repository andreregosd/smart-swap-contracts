...
error SmartSwapPool__PoolAlreadyInitialized();
error SmartSwapPool__PoolNotInitialized();
error SmartSwapPool__TokenTransferFailed();
error SmartSwapPool__NotEnoughLiquidity();
error SmartSwapPool__InvalidInputAmount();
error SmartSwapPool__InvalidOutputAmount();
error SmartSwapPool__NoLiquidity();
...
contract SmartSwapPool {
    ...
    function init(uint256 token0Amount, uint256 token1Amount) public returns(uint256) {
        if(totalLiquidity > 0)
            revert SmartSwapPool__PoolAlreadyInitialized();
    ...
    function getSwapOutput(bool token0ToToken1, uint256 inputAmount) public view returns(uint256) {
        if(inputAmount <= 0)
        ...
        if(token0Reserves <= 0 || token1Reserves <= 0)
            revert SmartSwapPool__NoLiquidity();

        // t0Reservers * t1Reserves = k and a 0.3% tax
        uint256 numerator = tokenReserves[1] * inputAmount * 997;
        uint256 denominator = tokenReserves[0] * 1000 + inputAmount * 997;
        return numerator / denominator;
    }

    function getSwapInput(bool token0ToToken1, uint256 outputAmount) public view returns(uint256) {
        if(outputAmount <= 0)
            revert SmartSwapPool__InvalidOutputAmount();
        ...
        uint256 numerator = tokenReserves[0] * outputAmount * 1000;
        uint256 denominator = tokenReserves[1] * 997 - outputAmount * 997;
    ...
    function addLiquidity(uint256 token0Amount) public {
        ...
        if(totalLiquidity == 0)
            revert SmartSwapPool__PoolNotInitialized();
    ...
    function swap(bool token0ToToken1, uint256 inputAmount) public {
        ...
        uint256 outputAmount = getSwapOutput(token0ToToken1, inputAmount);
